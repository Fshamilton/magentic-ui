...
class ContentGenerationTool:
    ...
    def generate_text(title_suggestion: Annotated[str, "Suggestion for the note title"],
        ...
        """
        logger.info(f"[ContentGenerationTool] Called generate_text for {target_platform}.")
        logger.debug(f"[ContentGenerationTool] Args: title_suggestion='{title_suggestion}', key_points={key_points}, persona='{persona}', tone='{tone}'")

        if not title_suggestion or not key_points:
            result = json.dumps({"status": "error", "message": "Text generation failed: Missing title suggestion or key points."})
            logger.warning(f"[ContentGenerationTool] generate_text failed due to missing inputs: title_suggestion='{title_suggestion}', key_points={key_points}")
            return result
        ...
        final_result_data = {"title": generated_title, "body": generated_body}
        final_result = json.dumps({"status": "success", "data": final_result_data})

        logger.info(f"[ContentGenerationTool] generate_text completed successfully for title suggestion '{title_suggestion}'.")
        logger.debug(f"[ContentGenerationTool] Generated text data: {final_result_data}")
        return final_result
    ...
    def generate_image(prompt: Annotated[str, "Visual concept prompt for image generation"],
        ...
        """
        logger.info(f"[ContentGenerationTool] Called generate_image.")
        logger.debug(f"[ContentGenerationTool] Args: prompt='{prompt}', style_hints='{style_hints}', aspect_ratio='{aspect_ratio}'")

        if not prompt:
            result = json.dumps({"status": "error", "message": "Image generation failed: Missing prompt."})
            logger.warning(f"[ContentGenerationTool] generate_image failed due to missing prompt.")
            return result
        ...
        if "fail_image_generation" in prompt:
            result = json.dumps({"status": "error", "message": "Image generation failed: Simulated API error"})
            logger.warning(f"[ContentGenerationTool] generate_image failed due to 'fail_image_generation' in prompt: '{prompt}'.")
            return result

        final_result_data = {"image_url": image_url, "alt_text_suggestion": alt_text}
        final_result = json.dumps({"status": "success", "data": final_result_data})

        logger.info(f"[ContentGenerationTool] generate_image completed successfully for prompt '{prompt}'.")
        logger.debug(f"[ContentGenerationTool] Generated image data: {final_result_data}")
        return final_result
